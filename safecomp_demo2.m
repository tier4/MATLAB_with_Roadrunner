...
if isempty(velocityData)
    velocityData = table('Size', [0, 4], 'VariableTypes', {'double', 'double', 'double', 'double'}, ...
        'VariableNames', {'Time', 'Sedan', 'Suv', 'Bus'});
end
...
allVelocityData = table('Size', [0, 5], 'VariableTypes', {'double', 'double', 'double', 'double', 'double'}, ...
...
for n=1:1
    ...
    while strcmp(get(rrSim,"SimulationStatus"),"Running")
        ...
        velocityAgent_Sedan = get(log,'Velocity','ActorID',2);
        velocityAgent_Suv = get(log,'Velocity','ActorID',3);
        velocityAgent_Bus = get(log,'Velocity','ActorID',4);

        currentTime = velocityAgent_Sedan(m);
        Time=currentTime.Time;
        currentSedan = velocityAgent_Sedan(m);
        ...
        % Time=currentTime.Time;
        Sedan_speed=currentSedan.Velocity;
        Suv_speed=currentSuv.Velocity;
        Bus_speed=currentBus.Velocity;

        Sedan_km=abs(Sedan_speed(2)*3.6);
        Suv_km=abs(Suv_speed(2)*3.6);
        Bus_km=abs(Bus_speed(1)*3.6);

        velocityTable2 = table(n,round(Time,1),round(Sedan_km,3), round(Suv_km,3), round(Bus_km,3), ...
            'VariableNames', {'n','Time','Sedan', 'SUV', 'Bus'});
         % ダッシュボードを表示/更新
